...
def create_static_trajectory_plot(data, exp_name, strain, treatment, output_dir):
    ...
    fig.suptitle(f'Worm Trajectories & Velocity - {strain} - {treatment}', fontsize=16, fontweight='bold')

    # Complete Trajectories with Longest Paths Highlighted (axes[0])
    # Calculate trajectory data for final frame visualization
    ...
    # Add text annotation
    axes[0].text(0.02, 0.98, f'Total: {len(valid_worm_ids)} trajectories',
                transform=axes[0].transAxes, fontsize=10, verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='white', edgecolor='black', alpha=0.8))

    # Velocity Distribution (axes[1])
    all_velocities = []
    for worm_id in worm_ids:
        worm_data = data[data['ID'] == worm_id].sort_values('FRAME')
        if len(worm_data) > 5:
            x, y, frames = worm_data['X'].values, worm_data['Y'].values, worm_data['FRAME'].values
            dx, dy, dt = np.diff(x), np.diff(y), np.diff(frames)
            dt[dt == 0] = 1
            velocities = np.sqrt(dx**2 + dy**2) / dt
            all_velocities.extend(velocities)
    all_velocities = np.array(all_velocities)
    if len(all_velocities) > 0:
        velocities_clean = all_velocities[all_velocities < np.percentile(all_velocities, 99)]
        axes[1].hist(velocities_clean, bins=50, alpha=0.7, color=PRIMARY_COLORS['blue'], edgecolor='black', linewidth=0.5)
        mean_vel, median_vel = np.mean(velocities_clean), np.median(velocities_clean)
        axes[1].axvline(mean_vel, color=PRIMARY_COLORS['red'], linestyle='--', linewidth=2, label=f'Mean: {mean_vel:.2f}')
        axes[1].axvline(median_vel, color=PRIMARY_COLORS['green'], linestyle='--', linewidth=2, label=f'Median: {median_vel:.2f}')
        axes[1].legend(frameon=True, fancybox=False, edgecolor='black')
    setup_plot_style(axes[1], title='Velocity Distribution', xlabel='Velocity (pixels/frame)', ylabel='Frequency')

    # Velocity Over Time (axes[2])
    velocity_by_frame = {}
    for worm_id in worm_ids:
        worm_data = data[data['ID'] == worm_id].sort_values('FRAME')
        if len(worm_data) > 5:
            ...
            for i, frame in enumerate(frames[:-1]):
                if frame not in velocity_by_frame:
                    velocity_by_frame[frame] = []
                velocity_by_frame[frame].append(velocities[i])
    if velocity_by_frame:
        frames_sorted = sorted(velocity_by_frame.keys())
        mean_velocities_by_frame = [np.mean(velocity_by_frame[f]) for f in frames_sorted]
        std_velocities_by_frame = [np.std(velocity_by_frame[f]) for f in frames_sorted]
        axes[2].plot(frames_sorted, mean_velocities_by_frame, color=PRIMARY_COLORS['blue'], linewidth=2, label='Mean')
        axes[2].fill_between(frames_sorted,
                            np.array(mean_velocities_by_frame) - np.array(std_velocities_by_frame),
                            np.array(mean_velocities_by_frame) + np.array(std_velocities_by_frame),
                            alpha=0.3, color=PRIMARY_COLORS['blue'])
        axes[2].legend(frameon=True, fancybox=False, edgecolor='black')
    setup_plot_style(axes[2], title='Velocity Over Time', xlabel='Frame', ylabel='Mean Velocity (pixels/frame)')
...
def main(skip_animations=False, workers=None):
    ...
    root_data_dir = Path('Comportamiento/Datos')
...
if __name__ == "__main__":
    ...
    if '--help' in sys.argv:
        print("Usage: python run_batch_analysis.py [options]")
        print("Options:")
